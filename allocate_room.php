	...
	<div class="container">
			<div >
				<form action="allocate_room.php" method="post">
					<div class="row">
					        <div class="col-md-6"> 
							<select name="hostelId" id="hostel" class="select_opt">
								<?php
									 $hostels_qu = "SELECT * FROM Hostel ";
									 $hostels_qu_result = mysqli_query($conn,$hostels_qu);
									 while($hostels_row = mysqli_fetch_assoc($hostels_qu_result)){
										// echo "<option>n </option>";
										if($hostel_id==$hostels_row['Hostel_id']){
											echo "<option value='{$hostels_row['Hostel_id']}' selected>{$hostels_row['Hostel_name']}</option>";
										}else{
											echo "<option value='{$hostels_row['Hostel_id']}'>{$hostels_row['Hostel_name']}</option>";
										}
							...
							<div class="col-md-3 mail_grid_w3l">
							<input type="submit" value="Select" name="assignHostel"></input>
							</div>
							<div class="col-md-3 mail_grid_w3l"></div>
					</div>
				</form>
			</div>
	</div>
         ...
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $student_name = $row7['Fname']." ".$row7['Lname'];
      		...
      		echo "<tr>
			<td>{$student_name}</td><td>{$row1['Student_id']}</td><td>{$hostel_name}</td><td>{$row1['Message']}</td>
