...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($username) || empty($password)) {
    header("Location: ../login-hostel_subwarden.php?error=emptyfields");
    exit();
  }
  else {
    ...
    if($row = mysqli_fetch_assoc($result)){
      ...
      if($pwdCheck == false){
        header("Location: ../login-hostel_subwarden.php?error=wrongpwd");
        exit();
      }
        ...
        if($_SESSION['isadmin']==0){
          header("Location: ../subwarden_home.php?login=success");
        }
        else {
      ...
      header("Location: ../login-hostel_subwarden.php?error=nouser");
      exit();
    }
  }
  ...
  header("Location: ../login-hostel_subwarden.php");
  exit();
}
